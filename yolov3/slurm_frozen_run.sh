...
do
  ...
  then
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 train_frozen.py  --weight_path=./weight/darknet53_448.weights &
  else
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 train_frozen.py  --weight_path=./weight/darknet53_448.weights 
  fi
done
