...
export ADAPTDL_MASTER_PORT=47021
...
do
  ...
  then
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 train.py  --weight_path=./weight/darknet53_448.weights &
  else
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 train.py  --weight_path=./weight/darknet53_448.weights 
  fi
done