...
mem_stats = dict()
mem_stats['total_memory'] = torch.cuda.get_device_properties(0).total_memory
with SummaryWriter(tensorboard_dir) as writer:
    ...
    for epoch, fronzen_layer_num in enumerate(frozen_list):
        mem_stats[fronzen_layer_num] = dict() 
        ...
        # for local_batch_size in [129]:
            ...
            if adaptdl.env.replica_rank() == 0: 
                ...
                mem_stats[fronzen_layer_num][local_batch_size] = torch.cuda.max_memory_allocated(0)
...
if adaptdl.env.replica_rank() == 0:
    print('saving speed info')
    with open('memory/model_{}.npy'.format(args.model), 'wb') as f:
        np.save(f, mem_stats) 