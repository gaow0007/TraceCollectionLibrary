...
export ADAPTDL_MASTER_PORT=43011
...
do
  ...
  then
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 -u mlm_task_adaptdl.py & #  --frozen & # --model=$model --bs=$batch_size --autoscale-bsz &
  else
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 -u mlm_task_adaptdl.py # --frozen # --model=$model --bs=$batch_size --autoscale-bsz
  fi
done
