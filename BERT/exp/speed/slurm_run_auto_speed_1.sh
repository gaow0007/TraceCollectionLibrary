...
export ADAPTDL_MASTER_PORT=47713
...
do 
    ...
    do
        ...
        then 
            # echo ${placement[@]}
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 -u mlm_task_speed.py --dataset=$dataset --placement=$placement_str &
        else
            # echo "$i"_"$node_id"
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 -u mlm_task_speed.py --dataset=$dataset --placement=$placement_str
        fi
