...
export ADAPTDL_MASTER_PORT=47009
...
do
  ...
  then
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 -u main.py --model=$model --bs=$batch_size --autoscale-bsz --frozen &
  else
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 -u main.py --model=$model --bs=$batch_size --autoscale-bsz --frozen
  fi
done