...
export ADAPTDL_MASTER_PORT=47064
...
do 
    ...
    do
        ...
        then 
            # echo ${placement[@]}
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 -u main_speed.py --model=$model --bs=$batch_size --placement=$placement_str &
        else
            # echo "$i"_"$node_id"
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 -u main_speed.py --model=$model --bs=$batch_size --placement=$placement_str
        fi
