...
export ADAPTDL_MASTER_PORT=42050
...
do
  ...
  then
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 -u test_cache_main.py --model=$model --bs=$batch_size --model=$model &
  else
    ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node python3 -u test_cache_main.py --model=$model --bs=$batch_size --model=$model
  fi
done
