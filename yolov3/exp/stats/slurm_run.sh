...
mkdir -p ./yolo/$worker_num/
export ADAPTDL_CHECKPOINT_PATH=./yolo/$worker_num/
...
do 
    ...
    do
        ...
        then 
            # echo ${placement[@]}
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 train_stats.py  --weight_path=./weight/darknet53_448.weights --epochs=$epochs &
        else
            # echo "$i"_"$node_id"
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 train_stats.py  --weight_path=./weight/darknet53_448.weights --epochs=$epochs
        fi
